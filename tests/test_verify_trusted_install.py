...
def test_verify_commit_signature_skipped_by_default(trusted_mod, monkeypatch):
...
def test_verify_commit_signature_requires_allowed_fingerprints(trusted_mod, monkeypatch):
...
def test_verify_commit_signature_uses_supported_git_invocation(trusted_mod, monkeypatch):
...
def test_verify_commit_signature_rejects_unapproved_signer(trusted_mod, monkeypatch):
    def fake_git(root: Path, *args: str) -> subprocess.CompletedProcess[str]:
        if args[:1] == ("verify-commit",):
            return subprocess.CompletedProcess(args, 0, "", "")
        if args[:2] == ("show", "-s"):
...
def test_main_logs_public_status_to_stdout(trusted_mod, monkeypatch, capsys):
    monkeypatch.setenv("ORAMA_TRUST_HERMES_SYNC", "1")

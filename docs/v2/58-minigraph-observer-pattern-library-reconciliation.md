...
its §8 observer-mechanism description where that text conflicts with the actual
`perpetua-core` PR #1 implementation and the recovered `GraphPlugin` groundwork.
...
This corrects the later prose in doc 57 and `01-kernel-spec.md` that claimed
`asteps()` itself was the scheduler and that `_run()` did not exist.

## 2. Why `asteps()` alone is not multicast

An async generator is a pull stream. One consumer drains each yielded item.
Two tasks racing over the same generator do not create broadcast semantics;
one can consume observations the other never receives.
...
The correct pattern is one drain plus deterministic push fan-out:
...
Current sequential semantics remain:
...
```text
R0/R1  canonical kernel reconciliation       implemented in core PR #1

...
| OQ29 | **GossipBus mesh transport** — Should v2.1 ship LAN-only `GossipMesh` (HTTP tail/ingest between orama + PT particles), or design BLE envelope now for bitchat-class offline mesh later? | v1 dogfoods file inbox + ws-peer; mesh gossip is the frugal event fan-out layer ([`43-gossipbus-mesh-transport.md`](43-gossipbus-mesh-transport.md)). BLE is optional v3; same JSON envelope either way. | v2.1 mesh module |
| OQ30 | **Mesh sync strategy** — Cursor-based tail pull (simple, frugal) vs CRDT/merge for partition tolerance? | Tail cursors match append-only GossipBus and hot-write batching. CRDT adds complexity; only needed if offline BLE multi-hop requires merge without central clock. | v2.1 design spike |

...
Rather than prompting an agent to "Fix this bug," we define a harness:
**"Write a failing test → Fix the bug → Run test again."** This makes a 90%-reliable LLM nearly
100% reliable by wrapping it in deterministic code.
...
```python
# Sentinel node enforcing the March of Nines
def sentinel_check(state: PerpetuaState):
    if not state.metadata.get("test_ran"):
        raise RuntimeError(
            "Sentinel Violation: Agent attempted implementation without a test run."
        )

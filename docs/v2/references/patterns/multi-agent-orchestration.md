...
Swarm treats orchestration as a series of transfers. Agent A returns a "Transfer" object that tells
the runner to switch context to Agent B.
...
CrewAI uses a "Manager" agent to break down tasks and assign them to workers. The manager maintains
the "Parent" context.
...
- **Mechanic**: The "Manager" is a \`MiniGraph\` that contains nodes for "Planning," "Delegation,"
  and "Aggregation."
- **Benefit**: Each step is deterministic and auditable via the \`GossipBus\`, preventing the
  manager from "hallucinating" the process.

## 3. AutoGen: "Nested Chats" & Multi-directional Flow

AutoGen allows agents to "talk it out" in group chats or spawn private side-chats to solve
sub-problems.

### oramasys v2 Adaptation: Sentinel-Gated Recursion

- **Mechanic**: We use **Recursive Node Calls**. A node can trigger a subgraph (Side-Chat) and wait
  for its completion before merging the results back into the main state.
- **Enforcement**: This is where the \`max_steps\` guard (Gemini Hardening 7b) is critical. It
  prevents agents from getting stuck in an infinite "nested chat" loop.

...
PT's `StateTransitionManager` (`orchestrator/state_transition_manager.py`, PR #205) implements a refcounted per-peer `asyncio.Lock` serializing `evaluate_observation` — the concrete mechanism underneath the P5/P6/P13 pattern mapping above. Its Phase 2 blockers doc (`Perpetua-Tools/docs/phase-0-specifications/2026-07-11-PHASE-2-BLOCKERS.md` § TODO #2) originally called for "benchmark latency impact (lock contention on high-peer-count deployments)" but that item is **deferred here** rather than done in PR #205: there is no concurrent multi-peer caller yet, and per the D23 premise check immediately above, a single-operator LAN may never produce the adversarial high-peer-count load the benchmark would need to be meaningful. Run this benchmark alongside SWARM's aggregate-risk-scoring rollout, once/if a real multi-operator deployment context exists — not before.

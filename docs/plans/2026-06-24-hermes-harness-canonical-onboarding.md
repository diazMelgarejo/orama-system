...
**Acceptance:** `grep -rn '192\.168\.' src/ scripts/ bin/ docs/` returns only fallback defaults inside resolution code. ⚠️ **This gate is RED today (M2):** it hits a real workstation IP `192.168.254.103` in [`optimization-priorities.md`](2026-06-24-optimization-priorities.md) (lines 32/39). That file is in scope and must be remediated (→ `$LM_STUDIO_WIN_ENDPOINT`) or the grep narrowed with an explicit, justified allowlist before Phase 1 can pass.

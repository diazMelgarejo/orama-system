...
`~/.claude/skills`) — but was never codified as an enforced rule in the
consolidation tool itself. A user request to "codify and formalize" it
surfaced that gap: knowing a rule happened once is not the same as the tool
refusing to repeat it. The exemption is scoped precisely: `skillify` (name
collision, proven incident) and the bare `gstack` slug (same class of risk,
pre-empted after this repo's own gstack-integration sub-skill was renamed to
the disambiguated `gstack-gbrain` on 2026-07-22) — `gstack-gbrain` itself is
NOT exempt, since it has no collision risk and was already safely wrapped
before this session.

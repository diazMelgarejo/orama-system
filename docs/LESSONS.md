...
branch's own OSSF-1 pilot work. Fetching the PR's *actual* merged head via
...
A direct tree diff between the local tip and the real merged head showed the
merge had dropped, relative to the branch's real work: the OSSF-1 pre-commit
...
and what actually *landed* (the merged head on `origin/main`), never the
...
is a full tree/stat diff against the actual `headRefOid`, not the assumed one.

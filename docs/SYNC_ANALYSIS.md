# Sync Analysis: orama-system â†” Perplexity-Tools

**Date:** 2026-04-11 | **Version:** ultrathink v0.9.9.9 Â· PT v0.9.9.9
...
## TL;DR â€” Status Summary

| Dimension | Status | Notes |
|---|---|---|
| **Version** | âœ… IN SYNC | Both at v0.9.9.9 |
| **Architecture contract** | âœ… IN SYNC | 4-layer hierarchy documented + upheld |
| **Bridge doc** | âœ… IN SYNC | PERPLEXITY_BRIDGE.md aligned to v0.9.9.9; HAL cross-link complete 
| **API endpoint spec** | âœ… IN SYNC | HTTP Bridge (`POST /oramasys`) is v1.0 RC primary transport; `/ultrathink` is a deprecated shim; MCP-Optional planned for v1.1 |
| **Idempotency contract** | âœ… RESOLVED | PT owns all state via `.state/agents.json`; ultrathink stateless (no Redis). Redis deferred to PT v1.1+ |
| **Shared `.env` contract** | âœ… IN SYNC | Vars in both `.env.example` files match BRIDGE doc |
| **SKILL.md cross-ref** | âœ… IN SYNC | PT SKILL.md references ultrathink routing methodology |
| **ECC Tools integration** | âœ… IN SYNC | Both agree on ECC as Stage-4 sub-agent selector |
| **autoresearch integration** | âœ… IN SYNC | Both reference uditgoenka/autoresearch equally |
| **Cross-repo README links** | âœ… IN SYNC | PT README links to orama-system; ultrathink README lists PT |
| **CI/CD** | âœ… IN SYNC | Both repos have `.github/workflows/ci.yml` with pytest + lint |
| **Tests** | âœ… RESOLVED | PT now has 6 test files (56+ tests); ultrathink has 86+ tests |
| **routing.yml** | âœ… IN SYNC | PT `config/routing.yml` has `deep_reasoning` + `code_analysis` ultrathink routes |
| **HAL doc cross-ref** | âœ… IN SYNC | PERPLEXITY_BRIDGE.md HAL section complete; 
| **PT hardware cross-link (OPT 1)** | âœ… RESOLVED | ultrathink `bin/orama-system/SKILL.md` references PT `hardware/SKILL.md`; `portal_server.py` and `network_autoconfig.py` are documented as active LAN helpers |
| **PT `pyproject.toml` (OPT 2)** | âœ… RESOLVED | PT now pip-installable as `perplexity-tools`; `[tool.pytest.ini_options]` + dev extras included |
| **Integration test suite (OPT 3)** | âœ… RESOLVED | `tests/test_ultrathink_integration.py` â€” 12 tests verifying routing.yml â†” ultrathink | **Integration test suite (OPT 3)** | âœ… RESOLVED | `tests/test_ultrathink_integration.py` â€” 12 tests verifying routing.yml â†” ultrathink contract |

## What IS Working (Synergized Well)

### 1. Architecture Hierarchy â€” Correct and Consistent

Both repos agree on the 4-layer stack:

```
Perplexity-Tools    â† top-level orchestrator, model selection, fallback chain
orama-system   â† reasoning engine, 5-stage methodology, CIDF
ECC Tools           â† Stage-4 parallel sub-agent auto-selection (up to 5x)
autoresearch        â† research automation, idempotent sync
```

Priority rule is identical in both READMEs. No conflict.

### 2. Version Parity â€” Both at v0.9.8.0

Aligned same day (2026-03-28). Releases match.

### 3. Cross-Reference Links â€” Complete

- PT README â†’ links orama-system (GitHub URL + role description)
- ultrathink README â†’ lists PT in "Compatible with" + architecture table
- PERPLEXITY_BRIDGE.md â†’ full integration doc with code examples

### 4. SKILL.md Routing Contract â€” Agreed
...
### 5. Fallback Chain â€” Documented and Consistent

```
PT receives task â†’ deep reasoning? â†’ call oramasys:8001
  â†’ timeout? â†’ local Qwen3:30b on Dell
  â†’ realtime/finance? â†’ Perplexity Grok 4.1
  â†’ simple Q&A? â†’ local Qwen3:8b
```

### 6. Tests â€” Both Repos Now Covered

**Perplexity-Tools** `tests/` (6 files, 56+ tests):
- `test_routing.py` â€” routing.yml contract, ultrathink routes, autoresearch routes
- `test_resilience.py` â€” connectivity resilience + fallback behaviour
- `test_lan_discovery.py` â€” LAN device discovery
- `test_agent_tracker.py` â€” 18 tests: AgentTracker lifecycle (register, update, find, conflicts, destroy)
- `test_cost_guard.py` â€” 16 tests: CostGuard budget, spend, alert, auto-reset, snapshot
- `test_autoresearch_bridge.py` â€” 22 tests: SwarmState parsing, GPU lock, preflight (all SSH mocked)

**orama-system** `tests/` â€” 86+ tests, full CI coverage.

### 7. CI/CD â€” Both Repos Active
...
### GAP 1: `api_server.py` â€” HISTORICAL BACKUP PATH
...
### GAP 5: PT Has No Tests â€” âœ… RESOLVED (v0.9.8.0)

6 test files now in `Perplexity-Tools/tests/` covering all critical orchestrator modules.

### GAP 6: PT Has No CI/CD â€” âœ… RESOLVED (v0.9.6.0)

`.github/workflows/ci.yml` created in PT with pytest + flake8 + YAML validation.

## Open Items (P2 / Future)

### OPT 1: ultrathink SKILL.md â†’ PT Hardware Profile Cross-Link

**Status:** âœ… RESOLVED (rolling â€” pre-v1.0 RC)
...
- **Do not change the priority rule.** PT SKILL.md â†’ ECC â†’ ultrathink ordering is correct.

## Update 2026-03-27: v0.9.5.0 Hardware Abstraction Layer [SYNC]

### Status: P0 & P1 Gaps RESOLVED âœ…
...
**P1 â€” RESOLVED:**
- âœ… orama-system `.env.example` updated with required API/model vars
- âœ… Perplexity-Tools `.env.example` updated with `ULTRATHINK_ENDPOINT`, `ULTRATHINK_TIMEOUT`, `ULTRATHINK_ENABLED`
- âœ… PT `config/routing.yml` now has `deep_reasoning` and `code_analysis` routes with ultrathink endpoint + fallback
...
- `hardware/SKILL.md` â€” Hardware profiles for `mac-studio` (Apple Silicon) and `win-rtx3080` (Dell RTX 3080)
  - Role-based model assignment matrix
  - VRAM/RAM safety rules
  - Fallback degradation chains
- `hardware/Modelfile.win-rtx3080` â€” Ollama Modelfile for Qwen3.5-35B-A3B with Flash Attention + KV cache compression
- `hardware/Modelfile.mac-studio` â€” Ollama Modelfile for Qwen3.5-9B manager agent with unified memory tuning
- `agent_launcher.py` â€” Hardware detection script with graceful degradation (Distributed â†’ Mac-only â†’ LM Studio â†’ Cloud)
  - Outputs routing state to `.state/agents.json`
  - 3-second timeout to avoid blocking
- `setup_wizard.py` â€” Idempotent installation wizard
...
- âœ… PT now has `tests/` with 6 test files (56+ tests)
- âœ… PT now has `.github/workflows/ci.yml`
- âœ… orama-system maintains 86+ tests + CI

### Recommended Next Actions

1. âœ… **Cross-link SKILL.md files** â€” DONE: ultrathink SKILL.md now references PT `hardware/SKILL.md` for hardware-aware model selection
2. âœ… **Add integration test** â€” DONE: `tests/test_hardware_routing.py` (commit `82cb179`) verifies PT correctly routes deep reasoning tasks to ultrathink with hardware-appropriate models (mac-studio â†’ MLX backend; win-rtx3080 â†’ Ollama backend).
3. :white_check_mark: **Hardware-agnostic core + model_hint passthrough** â€” IMPLEMENTED BACKUP NOTE: the HTTP `api_server.py` bridge accepts `model_hint`, but PT's `hardware/SKILL.md` remains the active hardware-routing source of truth.
...
- `UltraThinkRequest.model_hint: Optional[str]` â€” PT may inject a hardware-appropriate model name
...
- No breaking change â€” `model_hint` is optional; existing callers unaffected

**Status:** Implemented backup note â€” do not treat `api_server.py v0.9.9.0` as the active primary contract in this checkout.
...
- **MCP-Optional transport** (stdio JSON-RPC) renamed from "MCP-first" â€” planned for v1.1.
...
- Tier 1 (PT): `orchestrator/ultrathink_mcp_client.py`, `call_ultrathink_mcp_or_bridge()`, tests â€” in `PT/docs/ROADMAP_v1.1.md`.
- Tier 2 (ultrathink): `bin/shared/ollama_client.py`, `_solve()` real pipeline, tests â€” in `ultrathink/docs/ROADMAP_v1.1.md`.

...
**Architecture:** Two peer substrates. (1) `scripts/bind_codex_backend.sh` is the operational binding resolver: probe → native plugin (`codex-supervisor`) → idempotent install → OpenAI-compatible fallback provider → verify backend identity → record; it is the sole owner of `openclaw.json` provider and agent configuration. (2) `scripts/generate_codex_openclaw_profile.py` consumes the binding record + source profiles and writes the six OpenClaw directive files, `CODEX.md`, and refs, merging only marked generated sections. A thin `SKILL.md` orchestrates: mother skill → `openclaw-new-agent` overlay → bind → generate → stow → restart → assert.

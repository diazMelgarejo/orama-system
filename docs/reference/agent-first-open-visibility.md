...
| **`.mcp.json` location** | Committed [`orama-system/.cursor/mcp.json`](../../.cursor/mcp.json) — **code-review-graph + ai-cli-mcp** (stack: `bin/orama-system/config/cursor-mcp.stack.json`, sync: `scripts/sync-cursor-mcp.sh`). OpenClaw hub: [`OpenClaw/.mcp.json`](../../../.mcp.json). Reload MCP after pull. | `claude mcp` user registry + OpenClaw `.mcp.json` | [`OpenClaw/.mcp.json`](../../../.mcp.json) — CRG via `uvx code-review-graph serve` |
...
| **Rules / hooks / profiles** | [`orama-system/.cursor/rules/`](../../.cursor/rules/) (e.g. commit identity); repo hooks via [`scripts/git/install-local-hooks.sh`](../../scripts/git/install-local-hooks.sh) + [official identity policy](../wiki/08-git-hygiene-and-branching.md#official-commit-identity-policy-2026-05-25); See wiki § [Why only known @gmail.com](../wiki/08-git-hygiene-and-branching.md#why-only-known-gmailcom-in-co-author-lines).; no graph-before-Read hook yet | [`.claude/settings.json`](../../.claude/settings.json), [`.claude/skills/`](../../.claude/skills/), PreCompact hook via first-run | Profile bundle [`skills/code-review/profiles/J-drona23-v5/`](../../bin/orama-system/skills/code-review/profiles/J-drona23-v5/) (referenced from OpenClaw `CLAUDE.md`) |

...
Correction to the plan applied to the original combined spec: contracts moved from orama to PT (§0
...
- `JobSpec` (BaseModel) — immutable job descriptor (`model_config = ConfigDict(frozen=True)`);
  §5.1 fields: `role`, `specialization`,

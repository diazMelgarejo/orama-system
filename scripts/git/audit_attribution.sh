...
verboten_hit() {
  ...
  if [[ "$an_lc" == *REDACTED* || "$cn_lc" == *REDACTED* ]]; then return 0; fi
  if [[ "$ae_lc" == *REDACTED* || "$ce_lc" == *REDACTED* ]]; then return 0; fi
  local line
  while IFS= read -r line; do
    line_lc="$(printf '%s' "$line" | tr '[:upper:]' '[:lower:]')"
    case "$line_lc" in
      co-authored-by:*darth.serious*|co-authored-by:*REDACTED*) return 0 ;;
    esac
  done <<< "$body_lc"
  return 1
}

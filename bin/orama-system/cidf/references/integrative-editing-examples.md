...
| `ManagePullRequest` / `gh pr edit` without reading first | **READ → backup → write:** `gh pr view --json body -q .body > .git/pr-body-backups/<repo>-pr<N>-<ts>.md` then edit backup file, then `gh pr edit N --body-file` or `append-pr-body.sh` |

...
- **Paths: relative in-repo links + GitHub URLs only — NEVER absolute workstation paths.** A literal `/Users/<name>/…` or the `…/claude/OpenClaw` tree doxes the owner and fails CI (`scripts/review/repo_hygiene.py`). For **cross-repo** references prefer `https://github.com/diazMelgarejo/<repo>/blob/main/…` URLs (local agents resolve to a checkout); for **in-repo** references use relative paths. See [`oramasys-method/references/cross-repo-links.md`](../oramasys-method/references/cross-repo-links.md). For runtime paths use `$OPENCLAW_ROOT`/`$REPO_ROOT`/`$PERPETUA_TOOLS_PATH`/`~`. Same rule as the git-hygiene "no workstation paths" check.

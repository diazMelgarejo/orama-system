...
Classify incoming queries on type and audience level before oramasys routing or execution.

## When to Use

Before generating Type B (analytical), C (implementation), or D (ambiguous) responses in orama-system workflows.
...
| remembered "open PR" ⇒ reject a direct create-PR command as duplicate | is an **open** PR currently attached to this exact head repository owner/name, head SHA, and intended base — and if the prior PR is merged, does the head contain post-merge commits (if closed and never merged, does it have any delta at all)? | fetch live PR metadata and branch head; classify with the branch-state matrix in [`branch-local-pattern-remediation.md`](../references/branch-local-pattern-remediation.md) |

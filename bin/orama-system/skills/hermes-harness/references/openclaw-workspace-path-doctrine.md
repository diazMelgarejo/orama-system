...
| **ORAMA mother** | Git-relative crawl from orama root | `orama_root="$(git rev-parse --show-toplevel)" && dirname "$orama_root"` |
...
4. **Fail closed on crawl:** reject symlinked directories; collect every
   marker-valid checkout in the crawl direction; succeed only when exactly one
   candidate exists (otherwise error — do not pick the first match)
5. Fallback crawl from `$HOME` (shallow, git-marker only) — **not** `$OPENCLAW_HOME`
...
1. `$PERPETUA_TOOLS_PATH` / `$PT_HOME` / `$PERPETUA_TOOLS_ROOT` (aliases — see
   [`workspace-path-resolution.md`](workspace-path-resolution.md))

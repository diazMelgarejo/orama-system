...
- `ensure_local_mesh_secrets.py` → `GOSSIP_SHARED_SECRET` present in orama + PT `.env.local`
...
| Lane | Action |
|------|--------|
| `mac-orchestrator` | Secret in local `.env.local` — copy to **1Password** if not already |
| `win-cursor` | Paste **same** `GOSSIP_SHARED_SECRET` from 1Password into 3080 + 5080 `.env.local` (orama + PT) |
...
1. Save Mac `orama-system/.env.local` + `Perpetua-Tools/.env.local` in 1Password (secure note).

...
# scripts/lib/get-best-python.ps1 for why (a scheduled/long-running tick is
# not guaranteed to see the venv first on PATH).
...
while ((Get-Date) -lt $end) {
    ...
    try {
        ...
        & $PythonExe bin\orama-system\skills\hermes-harness\scripts\probe_lan_peer.py --json 2>&1 |
            Select-Object -First 8 | Tee-Object -FilePath $Log -Append

        & $PythonExe bin\orama-system\skills\hermes-harness\scripts\win_job_queue.py enqueue 2>&1 |
            Tee-Object -FilePath $Log -Append

        & $PythonExe bin\orama-system\skills\hermes-harness\scripts\lan_peer_assign.py list 2>&1 |
            Select-String -Pattern "win-autoresearcher|win-coder|coord-00" |
            Tee-Object -FilePath $Log -Append

        & $PythonExe bin\orama-system\skills\hermes-harness\scripts\lan_peer_assign.py list --peer 2>&1 |

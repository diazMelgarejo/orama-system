...
def reconcile_gemini(root: Path, archive_root: Path, only: set[str]) -> list[Path]:
    ...
    try:
        ...
        for slug in sorted(only):
            live = root / slug
            canonical_target = _canonical_target_for_slug(slug)
...
def main() -> int:
    ...
    if args.reconcile_gemini:
        ...
        if args.force_unlock:
            ...
            written = reconcile_gemini(HOME / ".gemini" / "skills", archive_root, only)
    ...
    if args.verify:
        if only and only & GEMINI_VERIFICATION_SLUGS:
            ...
            findings = verify_gemini(HOME / ".gemini" / "skills", archive_parent, only)
            if findings:

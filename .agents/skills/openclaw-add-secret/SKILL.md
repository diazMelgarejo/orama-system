...
- Canonical skill path (repo-relative): `bin/orama-system/skills/openclaw-skills/skills/openclaw-add-secret/SKILL.md`

## Before Use

Resolve the canonical repository root, in order, using the first candidate
whose `bin/orama-system/skills/openclaw-skills/skills/openclaw-add-secret/SKILL.md` exists as a file. Never hardcode a workstation path — search
...
3. A bounded, marker-based search of the current git repo's parent and
   grandparent directories (depth 2) for a sibling checkout containing `bin/orama-system/skills/openclaw-skills/skills/openclaw-add-secret/SKILL.md`
...
for cand in "$ORAMA_SYSTEM_ROOT" "$ORAMA_SYSTEM_PATH" \
  ...
  [ -n "$cand" ] && [ -f "$cand/bin/orama-system/skills/openclaw-skills/skills/openclaw-add-secret/SKILL.md" ] && ROOT="$cand" && break
done
if [ -z "$ROOT" ] && base="$(git rev-parse --show-toplevel 2>/dev/null)"; then
  parent="$(dirname "$base")"
  for d in "$parent"/*/ "$(dirname "$parent")"/*/; do
    [ -f "${d}bin/orama-system/skills/openclaw-skills/skills/openclaw-add-secret/SKILL.md" ] && ROOT="${d%/}" && break
  done
fi
```
...
Read `$ROOT/bin/orama-system/skills/openclaw-skills/skills/openclaw-add-secret/SKILL.md` and follow it. Do not copy behavior from this wrapper.
...
```bash
cd "$ROOT/bin/orama-system/skills/openclaw-skills/skills/openclaw-add-secret"

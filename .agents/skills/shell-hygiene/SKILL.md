---
name: shell-hygiene
description: "Safe shell command execution for agents. Covers enforced no-sleep-chain rules and zsh word-splitting behavior. Agents must use Monitor until-loops and run_in_background instead of sleep chains, and iterate command output with `while IFS=…"
---
...
whose `bin/orama-system/skills/shell-hygiene/SKILL.md` exists as a file. Never hardcode a workstation path — search
...
3. A bounded, marker-based search of the current git repo's parent and
   grandparent directories (depth 2) for a sibling checkout containing `bin/orama-system/skills/shell-hygiene/SKILL.md`
...
for cand in "$ORAMA_SYSTEM_ROOT" "$ORAMA_SYSTEM_PATH" \
  ...
  [ -n "$cand" ] && [ -f "$cand/bin/orama-system/skills/shell-hygiene/SKILL.md" ] && ROOT="$cand" && break
done
if [ -z "$ROOT" ] && base="$(git rev-parse --show-toplevel 2>/dev/null)"; then
  parent="$(dirname "$base")"
  for d in "$parent"/*/ "$(dirname "$parent")"/*/; do
    [ -f "${d}bin/orama-system/skills/shell-hygiene/SKILL.md" ] && ROOT="${d%/}" && break
  done
fi
```
...
Read `$ROOT/bin/orama-system/skills/shell-hygiene/SKILL.md` and follow it. Do not copy behavior from this wrapper.

...
future agent.** This repo's `main` has been **rewritten** (squash-rebundle / expunge / force-push).
After a rewrite every pre-rewrite commit keeps its content but gets a **new SHA**.
...
- Companion repo with the same protocol:
  ...
  **periscope is excluded** — its `main`/`agentsview` are pure upstream mirrors, never rewritten
  by us.
...
`feature/MacOS-post-install`. Contrib: `cursor/sync-attribution-guards-6421` → PR into integration.
`alphaclaw-align-all.sh` does not touch `pr-4-macos`. See
...
- **Existing skills to load:** use `bin/orama-system/skills/oramasys-method/SKILL.md` for
  ...
  `bin/orama-system/skills/oramasys-method/references/integrative-merge.md` for cross-branch/repo
  synthesis, and `bin/orama-system/skills/git-history-surgery/SKILL.md` before judging rewritten
  branch state. Before push after merge/cherry-pick/revert work, load

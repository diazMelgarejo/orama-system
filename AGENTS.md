...
- **Canonical implementation:** Perpetua-Tools owns `src/utils/endpoint_policy_core.py` and `config/endpoint-policy-contract.yml` on `main`.
- **Peer contract:** orama-system owns `config/endpoint-policy-contract.yml`, `scripts/security/check_endpoint_policy_contract.py`, and `.github/workflows/endpoint-policy-contract.yml` on `main`.

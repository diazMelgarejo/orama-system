"""Portal co-orchestration inbox view — bidirectional LAN peer file queue."""
...
<title>Co-orchestration inbox — orama portal</title>
<style>
  *{{box-sizing:border-box;margin:0;padding:0}}
  ...
  h1{{font-size:1.15rem;color:#38bdf8;margin-bottom:.75rem}}
  ...
  .md h1,.md h2,.md h3{{color:#38bdf8;margin:.75rem 0 .35rem}}
...
<body>
<nav class="navbar">
  <span class="nav-brand">orama co-orchestration</span>
  <div class="nav-links">
    <a class="nav-link" href="/">Portal home</a>
    <a class="nav-link active" href="/co-orchestration">Inbox queue</a>
    <a class="nav-link" href="/dashboard">Routing dashboard</a>
  </div>
</nav>
<h1>Mac ↔ Win file inbox <span style="font-size:.75rem;color:#64748b">v{version}</span></h1>
...
<div class="cols">
  <div class="panel">
    <div class="panel-h">Local inbox — received on this host</div>
    <div id="local-table"></div>
  </div>
  <div class="panel">
    <div class="panel-h">Peer inbox — on <span id="peer-label">peer</span></div>
...
<div class="footer">Auto-refresh 10s · <span id="last-refresh">—</span></div>
...
function rowHtml(item, side, idx) {{
  ...
  return '<tr data-side="' + side + '" data-idx="' + idx + '"' + sel + '>' +
    '<td>' + dirBadge(item, side) + esc(item.filename) + '</td>' +
...
def _annotate_inbox(
    ...
    *,
    ...
    """Add direction labels for the queue UI."""
...
def build_co_orchestration_summary(
    *,
    ...
    return {
        ...
        "peer_inbox": peer_ann,
        "stats": _summary_stats(local_ann, peer_ann, local_role=local_role, peer_role=peer_role),
...
FetchPeerInbox = Callable[[], Awaitable[tuple[list[dict[str, Any]], str]]]

...
`$env:ORAMA_SYSTEM_PATH = Join-Path $PWD ''
"@
    Set-Content -Path $PathsFile -Value $pathsContent -Encoding UTF8
# .paths.ps1 — auto-generated by .\platform\windows\start.ps1 on $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')
# Edit to override. Regenerate: Remove-Item .paths.ps1; .\platform\windows\start.ps1 --discover
...
if ($Glm52Available -and $env:GLM52_API_KEY) {
    ...
    if (-not (Test-Path "$HOME\\.alphaclaw")) {
        Write-Host '   Setup: see skills/glm52-fallback/SKILL.md or bin/orama-system/skills/glm52-fallback/SKILL.md'
    } else {
